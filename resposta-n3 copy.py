...
def destructure_array (outer_array, inner_array):
...
def recursion (graph, begin_vertex, end_vertex, visited = []):
    result = []
    one_step_search = one_vertex_search(graph, begin_vertex, end_vertex)
    visited.append(end_vertex)
    visited.append(begin_vertex)
    if one_step_search:
        return one_step_search
    else:
        for begin_adj in graph.get_adjlist()[begin_vertex]:
            if begin_adj in visited:
                continue
            for end_adj in graph.get_adjlist()[end_vertex]:
                if end_adj in visited:
                    continue
                if visited is not None:
                    result = visited
                recursion(graph, begin_adj, end_adj, visited)
    print(visited)
    print(result[:-2])
    return sort_arr(result[:-2])
print(str(recursion(g, 7, 2)))
